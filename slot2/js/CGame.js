function CGame(oData){
    ...
    this._init = function(){
		...
		this.getHashFromServer();
		playerSeed = this.generateUserSeed();
		_oInterface.refreshPlayerSeed(playerSeed);

		new Clipboard('.btn');

		hashModal.find('#closeButton').click(()=>{
			var ps = hashModal.find('#playerSeed').val();
			if(ps.length > 0)
			{
				playerSeed = ps;
				_oInterface.refreshPlayerSeed(playerSeed);
		...
		hashModal.find('#recordsButton').click(()=>{
			window.open('transactionRecords.php?address=' + web3.eth.defaultAccount, '_blank');
		});

		seedModal.find('#closeAndSkipButton').click(()=>{
			isSkippingSeedModal = true;
		});
		seedModal.find('#recordsButton').click(()=>{
			window.open('transactionRecords.php?address=' + web3.eth.defaultAccount, '_blank');
		});

		_oInterface.getHashBGShape().addEventListener('click', (event)=>{

			hashModal.find('#seedHash').val(seedHash);
			hashModal.find('#playerSeed').val(playerSeed);
    ...
    this.generateFinalSymbols = () => {
    	...
    	return new Promise((resolve, reject) => {
	        ...
	        console.log('_iLastLineActive: ' + _iLastLineActive);
	        console.log('WILD_SYMBOL: ' + WILD_SYMBOL);
	        console.log('NUM_REELS: ' + NUM_REELS);

	        var totalBet = _iLastLineActive * _iCurBet;
	        var bet = JSON.stringify({'lines': _iLastLineActive});

	   		ethGameContract.placeBet(bet, seedHash, playerSeed, {value: web3.toWei(totalBet, 'ether')}, (error, result)=>{
				console.log('result:' + result);

				var tx = result;				
				var amount = _iCurBet;
				var account = web3.eth.defaultAccount;
				var url = submitBetURL + "?seedHash=" + seedHash + "&playerSeed=" + playerSeed + "&tx=" + tx + "&bet=" + bet + "&amount=" + amount + "&from=" + account + "&slotType=2";
		        console.log('url: ' + url);
		        $.getJSON(url, (data) => {
		          ...
		          if(data['result'])
		          {
			        ...
			        for(var i=0;i<NUM_ROWS;i++){
			            ...
			            for(var j=0;j<NUM_REELS;j++){
			                ...
			                //console.log('s_aRandSymbols.length: ' + s_aRandSymbols.length);
			                var iRandIndex = results[i * NUM_REELS + j];
			        ...
			        console.log('_iTotWin: ' + _iTotWin);
					resolve(_aWinningLine.length);
		          }
		          else
		          {
		          	reject('false result');
    ...
    this.getHashFromServer = function()
	{
	    //$('#placeBetButton').attr('disabled', true);
	    //$('#seedHash').text('Loading...');
	    ...
	    $.getJSON(getHashURL, (data) => {
	      ...
	      //$('#placeBetButton').removeAttr('disabled');

	      _oInterface.refreshSeedHash(seedHash);
          _oInterface.enableSpin();
          
	      /*
	      var currentTS = Math.floor((new Date()).getTime() * 0.001);
	      hashExpireTimeCount = currentTS - parseInt(data['ts']);
	      console.log('hashExpireTimeCount: ' + hashExpireTimeCount);
	      startExpireCountDownUpdate();
			*/
	    });
	}

	this.generateUserSeed = function() {
	  var text = "";
    ...
    this._endReelAnimation = function(){
        ...
        if(!isSkippingSeedModal)
        {
	        setTimeout(()=>{
		        seedModal.find('#serverSeed').val(serverSeed);
    ...
    this.changeCoinBet = function(){
        var iNewBet = Math.round((_iCurBet+0.001) * 1000)/1000;
		var iNewTotalBet;
        ...
        if(iNewBet>MAX_BET){
			...
			_iCurBet = Math.round(_iCurBet * 1000) / 1000;
			_iTotBet = iNewTotalBet;
			_oInterface.refreshBet(_iCurBet);
			_oInterface.refreshTotalBet(_iTotBet);       
        }
    ...
    this.onSpin = function(){
        ...
        */
        ...
        var iRandSpin = Math.floor(Math.random() * 101);			
		//var bRet = this.generateFinalSymbols();
		this.generateFinalSymbols().then((winLineCount)=>{
    ...
    hashModal = oData.hashModal;
    seedModal = oData.seedModal;
    ethGameContract = oData.ethGameContract;
