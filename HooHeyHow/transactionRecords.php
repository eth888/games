  ...
  <body>
          ...
          <div class="modal-body">
            ...
            <div class="container-fluid">
                ...
                <div class="row" style="padding-top: 10px;">
                  <div class="col-md-12"><a href="https://jsbin.com/qidebel/edit?html,js,output" target="_blank" type="button" class="btn btn-info btn-block" id="verifyButton">Verify</a></div>
                </div>
      ...
      <div class="site-wrapper-inner">
        ...
        <div class="cover-container">
          ...
          <table class="table table-striped">
            <thead>
              <tr>
                <th style="text-align: center;">Stake (ETH)</th>                            
    ...
    $(document).ready(function() {
      ...
      $.getJSON( "getRecords.php?address=<?php echo $address; ?>", function(data) {
        ...
        {
          var row = data[i];
          var decisions = JSON.parse(row['decision']);

          var ds = '';
          for(var j = 0; j < decisions.length; j++)
          {
            var str = getSymbol(decisions[j]['number']);
          ...
          var rstr = getSymbol(results[0]) + ',' + getSymbol(results[1]) + ',' + getSymbol(results[2]);

          $('#recordList').append(element);

          var element = '<tr><td>' + row['amount'] + '</td><td>' + ds + '</td><td>' + rstr + '</td><td>' + row['payout'] + '</td><td>' + row['date'] + '</td><td>' + row['time'] + '</td><td><a href="#" onclick="onPopUp(' + i + ')"><i class="fa fa-check-square-o" style="font-size:24px"></i></a></td></tr>';
        }
    ...
    function onPopUp(index)
    {
      ...
      $('#hashModal').find('#verifyButton').text('Verify (' + data['result'] + ')');
