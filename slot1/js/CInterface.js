function CInterface(iCurBet,iTotBet,iMoney){
    ...
    this._init = function(iCurBet,iTotBet,iMoney){
	    ...
	    _oMoneyText = new createjs.Text(TEXT_MONEY +"\n"+iMoney.toFixed(3)+TEXT_CURRENCY,"24px "+FONT_GAME, "#ffde00");
        _oMoneyText.x = 408;
        _oMoneyText.y = 14;
        _oMoneyText.textAlign = "center";
        s_oStage.addChild(_oMoneyText);
        
        var graphics = new createjs.Graphics().beginFill("#333333").drawRoundRect(510, 4, 660, 68, 10, 8, 8, 8);
        _hashBGShape = new createjs.Shape(graphics);
        _hashBGShape.alpha = 0.6;
        s_oStage.addChild(_hashBGShape);

        _seedHashText = new createjs.Text('',"18px Arial", "#1100ff");
        _seedHashText.shadow = new createjs.Shadow('#ffffff', 1, 1, 0.6);
        _seedHashText.x = 536;
        _seedHashText.y = 18;
        _seedHashText.textAlign = "left";
        s_oStage.addChild(_seedHashText);

        _playerSeedText = new createjs.Text('aaa',"18px Arial", "#ff0011");
        _playerSeedText.shadow = new createjs.Shadow('#ffffff', 1, 1, 0.6);
        _playerSeedText.x = 536;
        _playerSeedText.y = 40;
        _playerSeedText.textAlign = "left";
        s_oStage.addChild(_playerSeedText);        
        ...
        _oCoinText = new createjs.Text(iCurBet.toFixed(3) ,"30px "+FONT_GAME, "#ffffff");
        _oCoinText.x =  712;
        ...
        _oTotalBetText = new createjs.Text(TEXT_BET +": "+iTotBet.toFixed(3),"30px "+FONT_GAME, "#ffffff");
    ...
    this.getHashBGShape = function()
    {
        return _hashBGShape;
    }

    this.refreshSeedHash = function(hash)
    {
        _seedHashText.text = hash;
    }

    this.getSeedHashText = function()
    {
        return _seedHashText;
    }

    this.refreshPlayerSeed = function(playerSeed)
    {
        _playerSeedText.text = playerSeed;
    }

    this.refreshMoney = function(iMoney){
        _oMoneyText.text = TEXT_MONEY +"\n"+iMoney.toFixed(3)+TEXT_CURRENCY;
    };
    
    this.refreshBet = function(iBet){
        _oCoinText.text = iBet.toFixed(3);
    };
    
    this.refreshTotalBet = function(iTotBet){
        _oTotalBetText.text = TEXT_BET +": "+iTotBet.toFixed(3);
    };
    ...
    this.refreshWinText = function(iWin){
        _oWinText.text = TEXT_WIN + " "+iWin.toFixed(3)+TEXT_CURRENCY;
    };
